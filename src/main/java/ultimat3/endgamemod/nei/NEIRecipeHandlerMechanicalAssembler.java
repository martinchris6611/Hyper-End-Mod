...
import static ultimat3.endgamemod.recipes.MechanicalAssemblerRecipes.output;
import static ultimat3.endgamemod.recipes.MechanicalAssemblerRecipes.recipe;
			...
			for (int i = 0; i < recipe.size(); i++) {
				...
				for(int x=0; x<4 && flag; x++) {
					for(int y=0; y<4 && flag; y++) {
						if(ingredient.isItemEqual(recipe.get(i)[x][y])) {
